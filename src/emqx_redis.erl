...
on_client_connected(Clinet = #{client_id := ClientId, username := Username}, ConnAck, ConnAttrs, _Env) ->
    io:format("Client(~s) Username(~s) connected, connack: ~w, conn_attrs:~p~n", [ClientId, Username, ConnAck, ConnAttrs]),
	AclCmd = "HMSET online device_%u %c",
	case emqx_redis_cli:q(AclCmd, Client) of
        {ok, _} -> ok;
